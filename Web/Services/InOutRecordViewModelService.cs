...
namespace Web.Services
{
    public class InOutRecordViewModelService:IInOutRecordViewModelService
    {
        ...
        public async Task<ResponseResultViewModel> GetInOutRecords(int? pageIndex,
            ...
            int? areaId,int? orderId,int? orderRowId,int? orderRowBatchId,int? pyId,string status,
            ...
            try
            {
                ...
                if (pageIndex.HasValue && pageIndex > -1 && itemsPage.HasValue && itemsPage > 0)
                {
                    baseSpecification = new InOutRecordPaginatedSpecification(pageIndex.Value, itemsPage.Value,
                        trayCode, materialName, type, ouId, wareHouseId, areaId, pyId, orderId, orderRowId,
                        orderRowBatchId, taskStatus,null, sCreateTime, eCreateTime);
                }
                else
                {
                    baseSpecification = new InOutRecordSpecification(trayCode, materialName, type, ouId, wareHouseId,
                        areaId,orderId, orderRowId, orderRowBatchId, pyId, taskStatus, null, null, sCreateTime, eCreateTime);
                }
                ...
                if (pageIndex > -1&&itemsPage>0)
                {
                    var count = await this._inOutRepository.CountAsync(new InOutRecordSpecification(trayCode,
                        materialName,type,ouId,wareHouseId,areaId,orderId,orderRowId,orderRowBatchId,pyId,taskStatus,

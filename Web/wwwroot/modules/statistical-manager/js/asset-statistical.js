...
$(function () {
    ...
    renderTree({rootId: 0,renderTarget:'jsTree',depthTag: 'ou',url:controllers.ou["get-ou-trees"],
        successCallback:function(rdata)
        {
            if(rdata.Children.length>0)
            {
               var id = rdata.Children[0].Id;
                ...
                $("#warehouse-title").text(name+"-"+"库存组织资产统计");
                $("#area-title").text(name+"-"+"子库存资产统计");
               loadChart(id);
            }
        ...
        selectNodeCall:function (node, data) {
            ...
            $("#warehouse-title").text(data.text+"-"+"库存组织资产统计");
            $("#area-title").text(data.text+"-"+"子库存资产统计");
            loadChart(ouId);
        },
        showRoot:true
    });
    ...
    {
        loadingShow();
        asynTask({type:'get',url:controllers.warehouse["warehouse-asset-chart"],
            jsonData:
                {
                    ouId:ouId
                },
        ...
        asynTask({type:'get',url:controllers["reservoir-area"]["area-asset-chart"],
            jsonData:
                {
                    ouId:ouId
                },
